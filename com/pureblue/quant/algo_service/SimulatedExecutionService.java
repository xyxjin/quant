...
import com.pureblue.quant.algo.IOrder;
import com.pureblue.quant.algo.IOrderStatusListener;
import com.pureblue.quant.algo.IPosition;
import com.pureblue.quant.algo.IPositionListener;
import com.pureblue.quant.algo.ISimulatedExecutionService;
import com.pureblue.quant.algo.ITrade;
import com.pureblue.quant.algo.OrderBean;
import com.pureblue.quant.algo.PositionBean;
import com.pureblue.quant.algo.TradeBean;
import com.pureblue.quant.dao.IStockDataCollection;
...
import com.pureblue.quant.model.ISeriesListener;
import com.pureblue.quant.model.ISeriesPoint;
import com.pureblue.quant.model.OrderSide;
import com.pureblue.quant.model.OrderType;
...
public class SimulatedExecutionService implements ISimulatedExecutionService {
	...
	private boolean processOrder(OrderInfo orderInfo) {
		...
		if (OrderSide.BUY.equals(orderInfo.order.getSide())) {
			//System.out.println(orderInfo.order.getContract());
			...
			//System.out.println("sell with actual");
	...
	private Double getLastPrice(IContract contract) {
		...
		if (listener != null) {
			System.out.println("LastPrice: " + listener.getLastPrice());
	...
	private Date getLastPriceTimestamp(IContract contract) {
		...
		if (listener != null) {
			...
			logger.log(Level.WARNING, "LastPriceTimestamp listener not found for contract: " + contract);
		}
		return null;
	}
	...
	public synchronized void setInputSeries(Collection<ISeries<Date, Double, ? extends ISeriesPoint<Date, Double>>> inputSeries) {
		for (ISeries<Date, Double, ? extends ISeriesPoint<Date, Double>> input : inputSeries) {
			...
			if (input instanceof IStockDataCollection) {
				//System.out.println("instanceof db");
			...
			if (contract != null) {
				//System.out.println("priceListerners start " + contract.getSymbol());
				priceListeners.put(contract, seriesListener);
				//System.out.println("priceListerners " + priceListeners.size() + " end");
			} 
